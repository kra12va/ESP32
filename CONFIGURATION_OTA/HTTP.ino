...
void handle_wifi_save() {
  /*
  Serial.println("отработала фунция handle_wifi_save"); 
  config.ssid = HTTP.arg("ssid");
  config.pwd = HTTP.arg("pwd");
  config.ap = false;
  Serial.println("Получены параметры: ");
  Serial.println("SSID: " + config.ssid);
  Serial.println("PWD: " + config.pwd);
  Serial.println("ap: " + config.ap);
